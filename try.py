...
class Test():
    ...
    # проверяем что количество товаров по запросу равно 8
    ...
    # проверяем что вверху верно отображено название
    ...
    # проверяем что в описании есть текст “В комплект входять 2 додаткових 'крила'"
    ...
    # проверяем, что есть 4 отзыва вверху
    ...
    # проверяем, что есть 4 отзыва, если нажать на видугки внизу
    def test_check_reviews_counter(self, driver):
        quantity = self.product_page(driver).\
            reviews_count()
        assert quantity == 4

    # залышыты видгук...мем надислаты видгук, форма не закрылась и что чекбокс подсвечен красным
